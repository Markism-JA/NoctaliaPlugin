...
=== #link(
  "https://noctalia.dev/plugins/community/keybind-cheatsheet",
)[Keybind Cheatsheet]

#figure(image("figures/cheatsheet-showcase.png"))
...
=== #link(
  "https://noctalia.dev/plugins/community/hypr-layout-switcher",
)[Hypr Layout Switcher]

#figure(image("figures/layout-switcher.png"))
...
This repository uses #link("https://lune-org.github.io/docs/")[Lune] alongside bundled type definitions (`types/*.d.luau`) to run test suites completely headless. Developers get instantaneous feedback on API contracts, type mismatches, and logic regressions directly from the terminal without touching a live compositor.

=== Architecture

#include "figures/fig-1.typ"
...
See more examples in the #link("https://github.com/Markism-JA/NoctaliaPlugin/tree/master/plugin/keybind-cheatsheet/tests")[keybind-cheatsheet test repository].

==== Test Runner Setup & Environment

Refer to the #link("https://lune-org.github.io/docs/getting-started/1-installation/")[official Lune installation guide] to install the binary for your platform.
...
#table(
  ...
  fill: (col, row) => if row == 0 { rgb("f0edf6") } else { none },
  [*Flag*], [*Description*],
  [`-f, --filter <str>`],
  [Filters suites and test descriptions matching the given substring.],

  [`-x, --fail-fast`],
  [Halts the test runner immediately on the first assertion failure.],

  [`-q, --quiet`],
  [Suppresses noisy error traces emitted by external processes during testing.],

  [`-h, --help`], [Prints the usage banner and flag reference.],
  [`[path ...]`],
  [Limits discovery to matching file paths, directories, or module names.],
)
...
- Luau & Type Bundler: #link("https://github.com/Markism-JA/nvim/blob/master/lua/plugins/luau.lua")[lua/plugins/luau.lua]
- TOML Manifest Validation: #link("https://github.com/Markism-JA/nvim/blob/master/lua/plugins/toml.lua")[lua/plugins/toml.lua]
...
#include "figures/fig-2.typ"

==== Extraction Mechanics

1. *Symbol Discovery via LuaL Registrations:*
  The extractor scans `kNoctaliaBaseLib`, `kWidgetLib`, `kPanelLib`, and nested module arrays in `luau_host.cc` and `plugin_bindings.cc`:
  ```regex
...
2. *Signature and Type Inference:*
...
- `luaL_checknumber` / `lua_tonumber` $arrow$ `number`
- `luaL_checklstring` / `lua_tolstring` $arrow$ `string`
- `lua_toboolean` $arrow$ `boolean`
- `luaL_checktype(..., LUA_TFUNCTION)` $arrow$ `(...any) -> (...any)`
- `luaToJson` / `jsonToLua` $arrow$ `any` (JSON-serializable table/primitive)
- `readUiTreeNode` / `luau_ui_render` $arrow$ `UiNode`
- Structured table readers (`httpRequestFromTable`, `tooltipRowFromLuaTable`) $arrow$ Luau record types (`HttpRequest`, `ScriptTooltipRowPatch`)


3. *Level Verification via Release Diffs:*
